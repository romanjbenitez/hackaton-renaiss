...
import { updateAgencyProperty } from "@/lib/agency/properties";
...
function redirectWithError(id: string, message: string): never {
  redirect(`/agency/properties/${id}/edit?error=${encodeURIComponent(message)}`);
}
...
export async function updatePropertyAction(formData: FormData) {
  ...
  if (!parsed.success) {
    redirectWithError(id, parsed.error.issues[0]?.message ?? "No se pudo guardar la propiedad.");
  }
  ...
  await updateAgencyProperty(id, agency.id, {
    title: data.title,
  ...
  redirect(`/agency/properties/${id}?message=Propiedad%20actualizada`);
}
